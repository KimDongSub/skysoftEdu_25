...
<tr style="width:50px; height:50px;">
	...
	<td style="text-align: center;">${srchVO.reviewerNm}<br><fmt:formatDate value="${srchVO.reviewerDtm}" pattern="yyyy/MM/dd"/></td>
	<td style="text-align: center;">${srchVO.approvalNm}<br><fmt:formatDate value="${srchVO.approvalDtm}" pattern="yyyy/MM/dd"/></td>
</tr>
</table>
<!-- end -->
	...
	<c:if test="${not empty list}">
		<c:forEach var="vo" items="${requestScope.list}">
			<c:if test="${vo.mngTypeCd=='00000001'}">
				${vo.mngGroupNm} / ${vo.mngNm}&nbsp;${vo.mngRankNm}
			</c:if>
		</c:forEach>
	</c:if>
	</td>
